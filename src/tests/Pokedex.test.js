...
describe('testando o componente <Pokedex.js />', () => {
  ...
  test('Verifica se o botão deve contém o texto `Próximo Pokémon` quando o botão é clicado ', () => {
    renderWithRouter(<Pokedex
      pokemonList={ pokemonList }
      isPokemonFavoriteById={ isPokemonFavoriteById }
    />);
  ...
  test('Verifica se é mostrado apenas um Pokémon por vez', () => {
    ...
    expect(pokemonCharmanderText).toBeInTheDocument();
    expect(pokemonCharmanderImg).toBeInTheDocument();
    expect(pokemonCharmanderText).not.toBe(/pikachu/i);
  });
  ...
  test('Verifica se a Pokédex contém um botão para resetar o filtro', () => {
    ...
    const textCaterpie = screen.getByText(/caterpie/i);
